...
<html>
	...
	<body>
	<h1> Congratulations <jsp:getProperty name="game" property="userName" /> You Entered Round 3 of the Best Secret Number Game  </h1>
	<hr>
		<% if((request.getAttribute("msg") == null) || (request.getAttribute("outofrange")!= null) || (request.getAttribute("note")!= null)){ %>
			...
			<p> 3<sup>rd</sup> Round - Take 3 Guesses Between <%= minimum %> and <%= maximum %> </p>
			<div id="right">
			...
			<form name="round" id= "round" action="${pageContext.request.contextPath}/GameServlet"  method="Get">
				...
				<input type="text" name="guess3" oninput="chkGuess3()" /> <br><br><br>
			
				<input type="submit" name="round3" value="Submit" /><br>
				</fieldset>
		<%}%>	
		
		<% if((request.getAttribute("msg") != null)&& request.getAttribute("gameovermsg") == null){ %>
			<p> 3<sup>rd</sup> Round - Your choosen numbers are : <% out.println(request.getAttribute("num1"));%>, 
		  ...
		  <p><%out.println(request.getAttribute("msg"));
			...
			<input type="submit" name="round3Continue" value="Continue" />
		<%}%>
